...
//        CSG.setDefaultOptType(CSG.OptType.POLYGON_BOUND);
        ...
        CSG mainPrism = new Cylinder(getRadius(), getThickness(), numEdges).toCSG().
                transformed(Transform.unity().translateZ(-getThickness() * 0.5).rotZ(initialRot));
        ...
        hinge1 = hinge1.transformed(Transform.unity().rotX(90));

        CSG pin = new Cube(getPinLength() + hingePrototype.getJointRadius(), getPinThickness(), getThickness()).toCSG().transformed(Transform.unity().
        ...
        hinge1 = hinge1.transformed(Transform.unity().
        ...
        for (int i = 1; i < numEdges; i++) {
            CSG hinge = hinge1.transformed(Transform.unity().rotZ(i * step));
            hinges.add(hinge);
        }

        CSG hingeHole1 = hinge1.transformed(Transform.unity().translateX(
                -apothem - hingePrototype.getJointRadius()
                - getPinLength()));
        hingeHole1 = hingeHole1.transformed(Transform.unity().scale(getHingeHoleScale()));
        hingeHole1 = hingeHole1.transformed(Transform.unity().translateX(
        ...
        for (int i = 1; i < numEdges; i++) {
            CSG hole = hingeHole1.transformed(Transform.unity().rotZ(i * step));
            hingeHoles.add(hole);
        }
        ...
        for (int i = 0; i < numEdges; i++) {
            if (i % 2 == 0) {
                ...
                combinedPart = combinedPart.difference(
                        hingeHoles.get(i).transformed(Transform.unity().rotZ(step)));
            }
        }
