...
//        resultNoStructure.toObj().toFiles(Paths.get("quadrocopter-arm-no-structure.obj"));
    ...
    private CSG toCSG() {
        ...
        CSG cross = new Cube(platformRadius*2, platformBorderThickness, platformThickness).toCSG().transformed(Transform.unity().translateZ(platformThickness/2.0));

        cross = cross.union(cross.transformed(Transform.unity().rotZ(90)));
    ...
    private CSG basePlatform(double platformRadius, int numHoneycombs, double platformThickness, double platformBorderThickness, double honeycombWallThickness) {
        ...
        return platform.union(platformShell,honeycombPrototype.transformed(Transform.unity().scale(1.05,1.05,1)));
    }
}
