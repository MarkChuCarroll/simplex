...
public class BreadBoardMount {
    ...
    private CSG board() {
        return new Cube(Vector3d.ZERO, Vector3d.xyz(breadBoardWidth, breadboardLength, breadBoardHeight)).toCSG();
    }
    ...
    private CSG pinConnections() {
        ...
        CSG first = new Cube(Vector3d.ZERO, Vector3d.xyz(breadBoardWidth / 2, 3, bottomThickness)).
                toCSG().transformed(Transform.unity().translate(-breadBoardWidth / 4, 0, bottomThickness / 2));
        CSG second = new Cube(Vector3d.ZERO, Vector3d.xyz(breadBoardWidth / 2 + 10, 3, bottomThickness)).
                toCSG().transformed(Transform.unity().rotZ(37.8).translate(breadBoardWidth / 4 + 5, 0, bottomThickness / 2));
        CSG third = new Cube(Vector3d.ZERO, Vector3d.xyz(breadBoardWidth / 2 + 10, 3, bottomThickness)).
                toCSG().transformed(Transform.unity().rotZ(-37.8).translate(breadBoardWidth / 4 + 5, 0, bottomThickness / 2));
    ...
    private CSG servoConnect() {
        ...
        CSG firstA = new Cube(Vector3d.ZERO, Vector3d.xyz(breadBoardWidth, servoConnectThickness, bottomThickness)).
                toCSG().transformed(Transform.unity().translate(0, -breadBoardHeight/2, bottomThickness / 2));

        CSG firstB = new Cube(Vector3d.ZERO, Vector3d.xyz(3, breadBoardHeight/2+servoConnectThickness/2, bottomThickness)).
