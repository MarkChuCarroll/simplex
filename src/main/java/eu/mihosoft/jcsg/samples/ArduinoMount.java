...
public class ArduinoMount {
    ...
    private CSG board() {
        return new Cube(Vector3d.ZERO, Vector3d.xyz(bottomWidth, bottomHeight, bottomThickness)).toCSG();
    }
    ...
    private CSG pinConnections() {
        ...
        CSG first = new Cube(Vector3d.ZERO, Vector3d.xyz(bottomWidth / 2, 3, bottomThickness)).
                toCSG().transformed(Transform.unity().translate(-bottomWidth / 4, 0, bottomThickness / 2));
        CSG second = new Cube(Vector3d.ZERO, Vector3d.xyz(bottomWidth / 2 + 10, 3, bottomThickness)).
                toCSG().transformed(Transform.unity().rotZ(37.8).translate(bottomWidth / 4 + 5, 0, bottomThickness / 2));
        CSG third = new Cube(Vector3d.ZERO, Vector3d.xyz(bottomWidth / 2 + 10, 3, bottomThickness)).
                toCSG().transformed(Transform.unity().rotZ(-37.8).translate(bottomWidth / 4 + 5, 0, bottomThickness / 2));
    ...
    private CSG servoConnect() {
        ...
        CSG firstA = new Cube(Vector3d.ZERO, Vector3d.xyz(bottomWidth, servoConnectThickness, bottomThickness)).
                toCSG().transformed(Transform.unity().translate(0, -bottomHeight/2, bottomThickness / 2));

        CSG firstB = new Cube(Vector3d.ZERO, Vector3d.xyz(3, bottomHeight/2+servoConnectThickness/2, bottomThickness)).
